...
const ReviewCourse = ({cart}) => {
    let [cartCourse,setCartCourse] = useContext(AddcourseContext)
    let {picture,title,price,rating,category,hours,instructor} = cart

    const removeCourse=(cart)=>{
        let rest = cartCourse.filter(carts=>carts.id!== cart.id)
            ...
            <div className="flex flex-col justify-between w-full pb-4">
                <div className="sm:flex justify-between w-full pb-2 space-x-2">
                    ...
                    <div className="sm:text-right mt-4 sm:mt-0">
                        ...
                        <p className="text-sm line-through dark:text-gray-600">{price}€</p>
                    </div>
                </div>
                <div className="flex text-sm divide-x">
                    <button onClick={()=>removeCourse(cart)} type="button" className="flex items-center px-2 py-1 pl-0 space-x-1">
